...
class Questionario:
    ...
    def to_csv(self, item):
        ...
        with open('pesquisa.csv', 'w'): # w de write
    ...
    def checkRespostas(self,chave, valor, dados):
        self.chave = chave
        self.valor = valor
        self.dados = dados
        pergunta = int(input(self.valor))
        ciclo = True

        while ciclo == True:

            if pergunta == 1:
                self.dados[self.chave].append('Sim')
                pass
            elif pergunta == 2:
                self.dados[self.chave].append('Não')
                pass
            elif pergunta == 3:
                self.dados[self.chave].append('Não soube responder')
                pass
            else:
                print('\nEscolha inválida, coloque um número que corresponde aos dados pelo sistema.')
                sleep(1)
                pesquisa.checkRespostas(self.chave, self.valor, self.dados)
            ciclo = False
    ...
    def checkGenero(self, chave, dados):
        self.chave = chave
        self.dados = dados 
        ciclo = True
        ...
        while ciclo == True:
            ...
            if genero == 1:
                self.dados[chave].append('Masculino')
            elif genero == 2:
                self.dados[chave].append('Feminino')
            elif genero == 3:
                self.dados[chave].append('Outros')
            elif genero == 4:
                self.dados[chave].append('Preferiu não informar')
            else:
                print('Escolha inválida!')
                pesquisa.checkGenero(self.chave, self.dados)
            ciclo = False
    ...
    def checkNome(self, chave, valor, dados ):
        self.chave = chave
        self.valor = valor
        self.dados = dados 
        nome = input(self.valor) 
        ciclo = True
        ...
        while ciclo == True:
            ...
            if self.chave == 'Nome':
                if nome not in dados[self.chave]:
                    self.dados[self.chave].append(nome)
...
def sistema():
    ...
    while sistema == True:
        ...
        print("=*"*60)
        sleep(1)
        print(f'<<< Adicionar Novo Registro >>> \n')
        ...
        if idade == 00:
            ...
            if os.path.exists('pesquisa.csv'): # Checa se o arquivo 'pesquisa.csv' já existe, caso sim, o programa lê o arquivo e aloja os valores no dicionario de dados.
                ...
                dados = {'Nome': df['Nome'].tolist(),
                'Gênero': df['Gênero'].tolist(),
                'Idade': df['Idade'].tolist(),
                'Pergunta 1':df['Pergunta 1'].tolist(),
                'Pergunta 2': df['Pergunta 2'].tolist(),
                'Pergunta 3':df['Pergunta 3'].tolist(),
                'Pergunta 4':df['Pergunta 4'].tolist(),
                'Hora':df['Hora'].tolist(),
                'Data': df['Data'].tolist()}   
            ...
            # Loop for que percorre os items do dicionarioDados, para depois chamar os métodos de validação de nome e gênero.
            for chave, valor in dicionarioDados.items():
                if chave == 'Gênero':
                    print(valor)
                    pesquisa.checkGenero(chave, dados)
                if chave == 'Nome':
                    pesquisa.checkNome(chave, valor, dados)
            ...
            for chave, valor in dicionarioPerguntas.items():
                pesquisa.checkRespostas(chave, valor, dados)
